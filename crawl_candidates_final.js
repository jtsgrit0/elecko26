...
function extractDistrict(text) {
    // 시/도 패턴
    const cityPattern = /(서울|부산|대구|인천|광주|대전|울산|세종|경기|강원|충북|충남|전북|전남|경북|경남|제주|경기도|강원도|충청북도|충청남도|전라북도|전라남도|경상북도|경상남도|제주특별자치도)/;
...
async function extractCandidateInfo(url) {
    try {
        ...
        for (const selector of possibleSelectors) {
            ...
            if (elements.length > 0) {
                ...
                elements.each((index, element) => {
                    ...
                    // 텍스트에서 한글 이름 추출
                    const name = extractKoreanName(elementText);
        ...
        if (!foundCandidates) {
            ...
            console.log(`  📊 ${candidateCount}명의 후보자 발견`);
        }
            ...
            try {
                ...
                for (const bioSelector of bioSelectors) {
                    ...
                    if (bioText && bioText.length > 10) {
                        candidate.bio = bioText.substring(0, 300);
                        break;
                    }
                }
