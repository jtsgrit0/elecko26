...
void main() async {
  ...
  try {
    ...
    await outputFile.writeAsString(JsonEncoder.withIndent('  ').convert(results));
...
Future<void> _saveIndividualCollections(Map<String, dynamic> results) async {
  ...
  for (final entry in results.entries) {
    ...
    await collectionFile.writeAsString(JsonEncoder.withIndent('  ').convert(entry.value));
    ...
    if (entry.key == 'members') {
      ...
      for (final memberEntry in membersData.entries) {
        final docFile = File('firebase_collections/members_documents/${memberEntry.key}.json');
        await docFile.writeAsString(JsonEncoder.withIndent('  ').convert(memberEntry.value));
      }
