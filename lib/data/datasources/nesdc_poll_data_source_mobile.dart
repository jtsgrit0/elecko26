...
class NesdcPollDataSource {
  ...
  Future<List<NesdcPollEntry>> fetchLatest(
    ...
    if (webOverride != null) {
      final fromBackend =
          await _fetchLatestFromBackend(pages, baseOverride: webOverride);
  ...
  Future<List<NesdcPollEntry>> _fetchLatestFromBackend(int pages,
      {Uri? baseOverride}) async {
  ...
  Future<NesdcPollDetail?> fetchDetail(String detailUrl) async {
    ...
    debugPrint('[NESDC] HTML body: $body');
    ...
    if (resultFileUrl != null) {
      ...
      if (fileResponse.statusCode == 200) {
        ...
        debugPrint(
            '[NESDC] PDF fetch failed: url=$resultFileUrl status=${fileResponse.statusCode}');
      }
    } else {
  ...
  Uri _resolveBackendBase() {
    final raw = AppConfig.kNesdcBaseUrl.trim();
    if (raw.isEmpty) {
  ...
  Uri _buildBackendUrl(Uri base, int pages) {
    ...
    final path =
        base.path.endsWith('/') ? '${base.path}polls' : '${base.path}/polls';
  ...
  NesdcPollEntry? _buildEntryFromColumns(List<String> cols, String? href) {
    ...
    final registeredDate =
        DateTime.tryParse(registeredDateText) ?? DateTime.now();
  ...
  DateTime? _parseSurveyDate(Map<String, String> fields, String? fallbackText) {
    // 1. "최초 공표" 날짜 형식을 최우선으로 파싱
    final announcementDate = _parseAnnouncementDate(fallbackText);
    if (announcementDate != null) {
      return announcementDate;
    }

    final candidates = [
      fields['조사기간'], // 붙여쓴 키를 최우선으로
      fields['조사 기간'],
      fields['조사일시'],
      fields['조사일'],
    ];
  ...
  DateTime? _parseAnnouncementDate(String? text) {
