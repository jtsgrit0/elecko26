...
class Election2018DataExtractor {
  ...
  static Map<String, Map<String, double>> extractPartySupportByDistrict(
      String electionText) {
    final districtPartySupport = <String, Map<String, double>>{};

    try {
      // 시/도별로 섹션 나누기
      final provincePattern =
      ...
      for (final provinceMatch in provinceMatches) {
        ...
        for (final districtMatch in districtMatches) {
          ...
          final peoplePowerMatches =
              peoplePowerPattern.allMatches(districtData);
          if (peoplePowerMatches.isNotEmpty) {
            final peoplePowerRate =
                double.parse(peoplePowerMatches.first.group(2)!);
            partySupport['국민의힘'] = peoplePowerRate / 100.0; // 0-1 범위로 정규화
          }

          // 기타 정당 지지율 계산
          final totalMajor =
              partySupport.values.fold(0.0, (sum, rate) => sum + rate);
          if (totalMajor < 1.0) {
  ...
  static Map<String, double> getPartySupportForDistrict(
      String district, String electionText) {
  ...
  /// 전국 평균 정당 지지율 계산
  static Map<String, double> calculateNationalAverage(
      Map<String, Map<String, double>> districtData) {
  ...
  static Map<String, dynamic> extractElectionMetadata(String electionText) {
    ...
    // 전국 평균 찾기
    final nationalAvgPattern =
        RegExp(r'전국 평균[\s\S]*?민주당:\s*(\d+\.\d+)%[\s\S]*?국민의힘:\s*(\d+\.\d+)%');
